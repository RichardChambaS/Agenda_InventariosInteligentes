...
public class PacienteServicio {
    ...
    public Pacientes[] getPacientesOrdenados() {
        Pacientes[] arr = new Pacientes[listaPacientes.size()];
        for (int i = 0; i < listaPacientes.size(); i++) {
            arr[i] = listaPacientes.get(i);
        }
        Arrays.sort(arr); // Usa compareTo de Pacientes: prioridad y apellido
        return arr;
    }
